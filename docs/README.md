...
项目的重点不是单张静态贴图，而是把角色动画帧、透明窗口穿透、输入控制、状态存储、行为调度、小游戏和 Linux/X11 截图贴图组合成一个完整的桌面体验。Python 脚本只用于素材处理、动作清单生成、Godot 运行时准备和 X11 全局快捷键桥接。
...
- **截图贴图** - Linux/X11 下支持 `F1` 区域截图、`F3` 轮换贴图、`F4` 关闭当前贴图
...
flowchart LR
  ...
  subgraph OS["Desktop OS"]
    ...
    X11["pet_hotkeys_x11.py<br/>UDP command bridge"]
    Tools["spectacle / import"]
  end
...
核心链路是：`Main.gd` 负责把窗口、动画、物理、输入、行为和小游戏接到一起；`PetPhysics.gd` 计算窗口位置；`PetSprite.gd` 渲染动作帧；截图贴图则通过 Godot 子窗口和 X11 全局快捷键辅助脚本扩展桌面能力。
...
| 快捷键 | X11 / `ctypes` / UDP | Linux/X11 全局快捷键触发截图贴图命令 |
| 截图后端 | KDE Spectacle / ImageMagick `import` | 区域截图、保存 PNG、复制到剪贴板 |
